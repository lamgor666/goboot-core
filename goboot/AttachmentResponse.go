...
func (p AttachmentResponse) AddSpecifyHeaders(arg0 interface{}) {
	...
	if ctx, ok := arg0.(*fiber.Ctx); ok {
		...
		ctx.Header(fiber.HeaderContentLength, fmt.Sprintf("%d", len(p.buf)))
		ctx.Header(fiber.HeaderTransferEncoding, "binary")
		ctx.Header(fiber.HeaderContentDisposition, disposition)
		ctx.Header(fiber.HeaderCacheControl, "no-cache, no-store, max-age=0, must-revalidate")
		ctx.Header(fiber.HeaderPragma, "public")
	}
}
